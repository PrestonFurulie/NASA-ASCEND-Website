...
2025-10-01
PC NASA ASCEND
Contributors: Angela Trainor, Marquis Muza, Ethan Pierson
...
- Added Adafruit BNO055 orientation sensor integration
- Added angular velocity and acceleration vector data collection
- Modified DataPoint struct to include orientation sensor data
- Updated header and data writing methods for new sensor data
- Added BNO055 test and setup functions
- Added readme; still incomplete
...
void setup() {
  ...
  Serial3.begin(9600);         // This should be `Serial3.begin(9600);` 
...
void writeHeader() {  // Write header(creates and writes header of csv file collumns)
  ...
  for (int attempt = 0; attempt < 3 && !isWritten; attempt++) {
    ...
    if (dataFile) {
      dataFile.println(" "
                      " ,elapsed(ms),UV_value1A,UV_value1B,UV_value1C,UV_value2A,UV_value2B,UV_value2C,UV_value3A,UV_value3B,UV_value3C, UV_value4A,UV_value4B,UV_value4C, pressure_bmp390, temp_bmp390, alt_bmp390, gyro_X, gyro_Y, gyro_Z, accel_X, accel_Y, accel_Z");
...
void writeDataPointToFile(DataPoint data, File file) {
  file.print(" ");  // Placeholder for UTC time
  file.print(",");
